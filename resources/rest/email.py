...
class EmailController(Resource):
    ...
    def get(self):
        self.logger.info('in list_emails')
        try: 
            ...
            self.logger.info('emailsJson:' + emailsJson)
            #response = json.loads(emailsJson)
            #self.logger.info('>>response loaded', response)
            #return response
...
class EmailByIdController(Resource):
    ...
    def get(self, id):
        self.logger.info('in list_emails with id:' + str(id))
        return http200okresponse
        
    def delete(self,id):
        self.logger.info('in delete:' + str(id))
